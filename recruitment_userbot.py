...
MEDIA_PHASE_SYSTEM = """Ты — Роберт, рекрутер-строитель. Данные кандидата уже собраны. Сейчас ты ждёшь от него фотографии работ (3-4 штуки).
ПРАВИЛА:
• Продолжай общаться в том же стиле — коротко, по-строительному, по-мужски.
• НЕ задавай вопросов про специализацию, документы, ставку — всё уже записано.
• Отвечай ОДНИМ коротким сообщением, максимум два предложения.
• Главная задача — не выдать себя за бота. Пиши как живой человек в телеграме.
• Иногда используй 👍 или 💪, но не переусердствуй.
• Не ставь всегда точки в конце. Пиши как на телефоне."""
    ...
    try:
        ...
        if msg.tool_calls:
            ...
            if tool_call.function.name == "save_candidate_data":
                ...
                if not final_msg:
                    final_msg = "Ок, записал. Скинь фотки работ, штуки 3-4" if photo_count == 0 else f"Записал. У тебя {photo_count} фото, можешь ещё докинуть или напиши готово"
                ...
                if event.photo or event.video or (event.document and "video" in getattr(event.document, "mime_type", "")):
                    ...
                    if event.grouped_id:
                        ...
                        async def send_debounced():
                            ...
                            situation = f"Кандидат прислал альбом фотографий. Всего у него теперь {count} фото. Подтверди получение и скажи что может ещё добавить или написать 'готово'."
                            reply = await generate_media_response(captured_uid, situation, history)
                            if not reply:
                                reply = f"Принял, {count} фоток. Ещё будут или готово?"
                        ...
                        situation = f"Кандидат прислал одно фото. Всего у него {count} фото. Подтверди получение."
                        reply = await generate_media_response(user_id, situation, user["conversation_history"])
                        if not reply:
                            reply = f"Принял, {count} фоток. Ещё будут?"
                        await human_typing_delay(user_id, reply)
